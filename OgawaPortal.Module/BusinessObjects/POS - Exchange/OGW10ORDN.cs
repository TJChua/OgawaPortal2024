...
namespace OgawaPortal.Module.BusinessObjects.POS___Exchange
{
    ...
    public class OGW10ORDN : XPObject
    {
        public OGW10ORDN(Session session)
            : base(session)
        {
        }
        public override void AfterConstruction()
        {
            ...
            ObjType = Session.FindObject<vwObjType>(CriteriaOperator.Parse("Code = 'OGW10ORDN'"));
        ...
        public decimal Cash
        {
            get
            {
                if (Session.IsObjectsSaving != true)
                {
                    decimal rtn = 0;
                    if (OGW12ORDN != null)
                        rtn += OGW12ORDN.Sum(p => p.CashAmount);
        ...
        public decimal CreditCard
        {
            get
            {
                if (Session.IsObjectsSaving != true)
                {
                    decimal rtn = 0;
                    if (OGW12ORDN != null)
                        rtn += OGW12ORDN.Sum(p => p.CreditCardAmount);
        ...
        public decimal Voucher
        {
            get
            {
                if (Session.IsObjectsSaving != true)
                {
                    decimal rtn = 0;
                    if (OGW12ORDN != null)
                        rtn += OGW12ORDN.Sum(p => p.VoucherAmount);
        ...
        public XPCollection<OGW11ORDN> OGW11ORDN
        {
            get { return GetCollection<OGW11ORDN>("OGW11ORDN"); }
        }

        [Association("OGW10ORDN-OGW12ORDN")]
        [XafDisplayName("Payment")]
        [Appearance("OGW12ORDN", Enabled = false, Criteria = "IsNew")]
        public XPCollection<OGW12ORDN> OGW12ORDN
        {
            get { return GetCollection<OGW12ORDN>("OGW12ORDN"); }
        }

        [Association("OGW10ORDN-OGW15ORDN")]
        [XafDisplayName("Document Status")]
        public XPCollection<OGW15ORDN> OGW15ORDN
        {
            get { return GetCollection<OGW15ORDN>("OGW15ORDN"); }
        }
